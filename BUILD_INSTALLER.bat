...
if not exist "dist\TocaDoCoelho\TocaDoCoelho.exe" (
    ...
    echo   pyinstaller --noconfirm --onedir --name TocaDoCoelho --icon coelho_icon_transparent.ico --collect-binaries imageio_ffmpeg --collect-all faster_whisper --collect-all ctranslate2 --collect-all whisper launcher.py
    echo.
    pause
    exit /b 1
)
