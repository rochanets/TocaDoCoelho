...
if not exist "dist\TocaDoCoelho\TocaDoCoelho.exe" (
    ...
    echo   pyinstaller --noconfirm --onedir --windowed --name TocaDoCoelho --icon coelho_icon_transparent.ico --add-data "app.py;." --add-data "public;public" --collect-binaries imageio_ffmpeg --collect-all faster_whisper --collect-all ctranslate2 --hidden-import app launcher.py
    echo.
    pause
    exit /b 1
)
