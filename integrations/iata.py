...
# ---------------------------------------------------------------------------
# Prompt de extração e parsing da hierarquia (Task 3)
# ---------------------------------------------------------------------------
...
def _loads_tolerante(raw):
    ...
    código markdown (com ou sem a tag `json`), texto explicativo antes/depois
    do objeto, e aspas tipográficas usadas como delimitador de string. Não
    tenta recuperar JSON truncado — nesse caso devolve None (o chamador trata
    como falha de extração, não inventa dado)."""
    ...
    # Última tentativa: reparo de aspas curvas, sobre o trecho já recortado.
...
def _as_item_list(value, name_key='name'):
    ...
    Tolera três desvios comuns de um LLM em relação ao esquema pedido:
    ...
    Sem isso, um `for x in dict` itera as CHAVES do dict como se fossem
    itens (silenciosamente descartados no `isinstance(x, dict)` seguinte),
    apagando a hierarquia inteira sem nenhum sinal de erro.
    """
    if value is None:
        return []
...
def _parse_participants(raw_participants):
    ...
    if isinstance(raw_participants, str):
        # Um LLM pode devolver "Ana, Bruno" em vez de uma lista — iterar uma
        # string sem tratar aqui percorreria caractere a caractere.
